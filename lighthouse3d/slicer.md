...
| **M6 hex nut pockets** | platform underside, ×3 (lamp mount) | Ø12 hex × **5.6 mm deep**, opens **downward**; central Ø6.6 hole; **roof chamfered to a ~48° cone** | **No support** — the conical roof is self-supporting printed bottom-up. The captive press-fit nut seats on the cone (fine for a light beacon). |

So in practice you **don't** need dissolvable filament for the nuts at all — the M3 slots bridge,
the inserts are straight holes, and the M6 pockets now have a **self-supporting chamfered roof**
(done in [`latarnia_cadquery.py`](latarnia_cadquery.py); the flat ledge was replaced by a ~48° cone).
